// app/src/main/java/com/lazymohan/zebraprinter/grn/ui/TransferGrnViewModel.kt
...
enum class ToStep { ENTER_TO, SHOW_TO, REVIEW, SUMMARY }

data class ToUiState(
    val step: ToStep = ToStep.ENTER_TO,
...
class TransferGrnViewModel @Inject constructor(
    ...
    val state: StateFlow<ToUiState> = _state
    ...
    fun fetchTransferOrder() = viewModelScope.launch {
        ...
        repo.fetchTransferOrder(s.toNumber)
            .onSuccess { header ->
                ...
                _state.value = _state.value.copy(loading = false, header = header, step = ToStep.SHOW_TO)
    ...
    private fun TransferOrderLineItem.toLineInput(initial: LineSectionInput? = null) = LineInput(
        ...
        maxQty = Double.MAX_VALUE, // adjust if you fetch remaining qty
            ...
            .onSuccess { items ->
                ...
                if (extracted.isNotEmpty()) {
                    ...
                    val matchedInputs = items.mapNotNull { line ->
                        val matchIdx = bestMatchIndex(extracted, line.ItemNumber.trim(), used, 0.01)
                        if (matchIdx != null) {
                            used += matchIdx
    ...
    fun buildTOPayloadsAndReview() = viewModelScope.launch {
        ...
        for (secIdx in 1..maxSection) {
            val linesForThisSection = s.lines.mapNotNull { line ->
                ...
                TransferReceiptLine(
                    ...
                    OrganizationCode = BuildConfig.TRANSFER_TO_ORG, // required
                    Quantity = sec.qty,
            ...
            if (linesForThisSection.isNotEmpty()) {
                staged += ToStaged(
                    ...
                    request = TransferReceiptRequest(
                        ...
                        OrganizationCode = BuildConfig.TRANSFER_TO_ORG,
        ...
        _state.value = s.copy(progress = progress, step = ToStep.REVIEW)
    }

    fun backToReceive() { _state.value = _state.value.copy(step = ToStep.SHOW_TO) }
    ...
    fun prefillFromScan(toNumber: String?, scanJson: String?, cachePaths: List<String>) {
        ...
        _state.value = next
        if (_state.value.toNumber.isNotBlank() && _state.value.step == ToStep.ENTER_TO) {
            fetchTransferOrder()
        }
    }
}
