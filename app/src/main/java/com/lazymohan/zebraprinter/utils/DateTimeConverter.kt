...
class DateTimeConverter(
    ...
    fun getUIDisplayDate(input: Date?): String {
        if (input == null) {
            return ""
        }
        return try {
            val simpleDateFormat = SimpleDateFormat(dateTimeFormat.uiDateFormat, Locale.getDefault())
            simpleDateFormat.timeZone = TimeZone.getDefault()
            simpleDateFormat.format(input)
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    fun getGS1DisplayDate(input: Date?): String {
        if (input == null) {
            return ""
        }
        return try {
            val simpleDateFormat = SimpleDateFormat(dateTimeFormat.gs1dateFormat, Locale.getDefault())
            simpleDateFormat.timeZone = TimeZone.getDefault()
            simpleDateFormat.format(input)
        } catch (e: Exception) {
            e.printStackTrace()
